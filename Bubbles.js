+function(){
	// 浏览器辅助
	var _VER_ = navigator.userAgent; //Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Safari/537.36
	
	var _IE6_ = /IE 6/.test(_VER_);  //是否是IE 6浏览器 fasle
	
	var STD = !!window.addEventListener; //判断浏览器是否支持window.addEventListener
	var de = document.documentElement;  //返回文档对象的根元素，即html
	
	// document.execCommand("BackgroundImageCache", false, true)  IE的cache设置为Every visit to the page，而不是默认的Automatically。IE6下设置背景图片是不会被真正cache住的.但一般只有开发者才会把cache设置为每次访问检查更新，所以这个bug不会影响到真正的用户。
